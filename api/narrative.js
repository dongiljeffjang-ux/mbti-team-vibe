...
export default async function handler(req, res) {
  ...
  const cacheKey = crypto
    .createHash("sha256")
    .update(JSON.stringify({ v: 12, model: MODEL, teamName, goal, purpose, industry, companyCulture, leadership, members: members.map((m) => [m.name, m.type, m.axes, m.role, m.note, m.birthDate, m.birthTime, m.birthCalendar, m.gender]) }))
    .digest("hex");
  ...
  try {
    ...
    if (!strengths.executiveInsight || !strengths.summary || !strengths.organizationCulture || !strengths.teamCulture || !Array.isArray(strengths.strengths)) throw new Error("종합 인사이트 응답 형식이 올바르지 않습니다.");
