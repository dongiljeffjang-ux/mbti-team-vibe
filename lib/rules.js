...
export function axisScoresForMember(member) {
  if (!member?.axes) return axisScores(member?.type || "ISTJ");
  const p = { E: Number(member.axes.ei) / 100, I: 1 - Number(member.axes.ei) / 100, N: Number(member.axes.sn) / 100, S: 1 - Number(member.axes.sn) / 100, F: Number(member.axes.tf) / 100, T: 1 - Number(member.axes.tf) / 100, P: Number(member.axes.jp) / 100, J: 1 - Number(member.axes.jp) / 100 };
  const raw = Object.fromEntries(AXIS_LIST.map((axis) => [axis, Object.entries(p).reduce((sum, [letter, weight]) => sum + (AXIS_RULES[axis][letter] || 0) * weight, 0)]));
  const total = AXIS_LIST.reduce((sum, axis) => sum + raw[axis], 0) || 1;
  return Object.fromEntries(AXIS_LIST.map((axis) => [axis, (raw[axis] / total) * 100]));
}
...
export function dimRatios(types) {
  ...
  types.forEach((item) => {
    if (typeof item === "string") item.split("").forEach((l) => (c[l] += 1));
    else { const a = item.axes || axesFromType(item.type); c.E += Number(a.ei) / 100; c.I += 1 - Number(a.ei) / 100; c.N += Number(a.sn) / 100; c.S += 1 - Number(a.sn) / 100; c.F += Number(a.tf) / 100; c.T += 1 - Number(a.tf) / 100; c.P += Number(a.jp) / 100; c.J += 1 - Number(a.jp) / 100; }
  });
  const r = {};
...
export function teamAxisMix(types) {
  ...
  types.forEach((t) => {
    const a = typeof t === "string" ? axisScores(t) : axisScoresForMember(t);
...
export function analyze(members) {
  ...
  const { ratio, counts } = dimRatios(members);
  const pairs = [];
