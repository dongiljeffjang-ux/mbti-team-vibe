        ...
        <div className="roster">
          {members.map((m, i) => (
            <div className="roster-row" key={m.id}>
              ...
              <div className="saju-fields"><span>사주 참고</span><input type="date" min="1950-01-01" max={TODAY} value={m.birthDate || (m._dateAnchor ? "2000-01-01" : "")} aria-label={`${m.name || "팀원"} 생년월일`} onFocus={() => { if (!m.birthDate) update(m.id, { birthDate: "2000-01-01", _dateAnchor: true }); }} onChange={(e) => update(m.id, { birthDate: e.target.value, _dateAnchor: false })} onBlur={() => { if (m._dateAnchor) update(m.id, { birthDate: "", _dateAnchor: false }); }} /><input type="time" value={m.birthTime || ""} aria-label={`${m.name || "팀원"} 생시`} onChange={(e) => update(m.id, { birthTime: e.target.value })} /><select value={m.birthCalendar || "solar"} onChange={(e) => update(m.id, { birthCalendar: e.target.value })}><option value="solar">양력</option><option value="lunar">음력</option></select><select value={m.gender || "unknown"} onChange={(e) => update(m.id, { gender: e.target.value })}><option value="unknown">성별 미입력</option><option value="male">남성</option><option value="female">여성</option></select></div>
